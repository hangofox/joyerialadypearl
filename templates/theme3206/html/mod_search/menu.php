...
?>
<i class="search fl-outicons-magnifying"></i>
<div role="search" class="mod-search mod-search__<?php echo $moduleclass_sfx ?>">
  	...
  	<?php
		$output = '<label for="searchword" class="element-invisible">' . $label . '</label> <input id="searchword" name="searchword" maxlength="' . $maxlength . '"  class="inputbox mod-search_searchword" type="text" size="' . $width . '" placeholder="' . $text . '" required>';
			...
			if ($imagebutton) :
				...
				$btn_output = ' <button class="button btn btn-primary" onclick="this.form.searchword.focus();"><i class="search"></i>' . $button_text . '</button>';
			endif;
  ...
  <i class="close fl-outicons-cancel29"></i>
</div>